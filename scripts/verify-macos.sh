#!/bin/bash
# macOS 端到端验证：确认这台 Mac 能不能把游戏的技术脊椎跑起来，并留下完整日志。
#
...
#   4. 客户机镜像
# 所有输出落到 m0/run/verify-macos-<时间戳>/，最后打成同名 zip，把 zip 发回来即可。
#
# 用法：
#   scripts/verify-macos.sh                 只检查、不装任何东西
#   scripts/verify-macos.sh --install       缺什么装什么（不需要 sudo）：
#                                             .NET 10 SDK -> ~/.dotnet
#                                             QEMU        -> brew install qemu
#                                             Godot mono  -> ~/Applications/Godot_mono.app
#                                             godot-xterm -> 项目 addons/
#   scripts/verify-macos.sh --godot /path/to/Godot_mono.app --qemu /path/to/qemu-system-x86_64
#   scripts/verify-macos.sh --offline       不联网（插件缺失时直接报错）
#
# 客户机镜像只能在 Linux 上构建：在 Linux 上跑 scripts/pack-for-windows.sh，
# 把产出的 zip 解到仓库根即可（包内容与平台无关，名字是历史原因）。
#
...
OUT="$REPO/m0/run/verify-macos-$STAMP"
mkdir -p "$OUT"
...
step 1 '环境信息已记录'
if [ "$ARCH" = arm64 ]; then
    echo '  注意：Apple Silicon 上 x86_64 客户机没有硬件加速（HVF 只加速 arm64 客户机），纯 TCG'
fi

# ---------------------------------------------------------------------------
section '2. .NET SDK'
# dotnet-install.sh 默认装到 ~/.dotnet，官方 pkg 装到 /usr/local/share/dotnet，
# 两处都不一定在 PATH 上（尤其是 ssh 进来的非登录 shell）
DOTNET=""
find_dotnet() {
    local c
    for c in "$(command -v dotnet 2>/dev/null)" "$HOME/.dotnet/dotnet" /usr/local/share/dotnet/dotnet; do
        [ -n "$c" ] && [ -x "$c" ] || continue
...
if [ -n "$DOTNET" ]; then
    ...
    export DOTNET_ROOT="$(dirname "$DOTNET")"      # Godot 靠它找到 ~/.dotnet 里的运行时
...
for c in "$QEMU_ARG" "${GAMEHACKER_QEMU:-}" \
         "$REPO/runtime/macos-$ARCH/bin/qemu-system-x86_64" \
...
if [ -z "$QEMU" ]; then
    ...
    # 从 Finder 启动的程序拿不到 /opt/homebrew/bin，所以显式交给它
    export GAMEHACKER_QEMU="$QEMU"
...
if [ -n "$MISSING" ]; then
    ...
    step 1 '客户机镜像' "$(cd "$REPO/m0/images" && ls -l vmlinuz-virt m0-guest.cpio.gz alpine-main.qcow2 \
                          | awk '{printf "%s=%.1fMB ", $9, $5/1048576}')"
fi
...
resolve_godot() {
    ...
    for c in "$GODOT_ARG" "$GODOT_ENV" /Applications/Godot_mono.app "$HOME/Applications/Godot_mono.app" \
             "$HOME/gh-setup/Godot_mono.app" "$(command -v godot-mono 2>/dev/null)" "$(command -v godot 2>/dev/null)"; do
        [ -n "$c" ] || continue
        # 给 .app 的话取里面真正的可执行文件
...
if [ -z "$GODOT" ] && [ "$INSTALL" = 1 ] && [ "$OFFLINE" = 0 ]; then
    ...
    curl -fsSL -o "$OUT/godot.zip" \
        "https://github.com/godotengine/godot/releases/download/$GODOT_VERSION-stable/Godot_v$GODOT_VERSION-stable_mono_macos.universal.zip" \
        && unzip -q -o "$OUT/godot.zip" -d "$HOME/Applications" && rm -f "$OUT/godot.zip"
    resolve_godot
fi

if [ -z "$GODOT" ]; then
    step 0 'Godot mono' "找不到。加 --install，或 --godot 指向 Godot_mono.app"
else
    ...
    if [ -d "$ADDON" ] && xattr -r "$ADDON" 2>/dev/null | grep -q com.apple.quarantine; then
        if [ "$INSTALL" = 1 ]; then
    ...
    if [ -z "$DOTNET" ]; then
        ...
        unset GAMEHACKER_SELFTEST GAMEHACKER_PCAP
    fi
...
section '7. 残留进程'
sleep 2
ORPHANS="$(pgrep -f qemu-system | wc -l | tr -d ' ')"
